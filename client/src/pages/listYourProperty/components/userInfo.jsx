...
function UserInfo({returnedGoNext}) {
   ...
   // Simplify useEffect
    ...
    return ( <form onSubmit={handleSubmit(onSubmit)} className="w-full">
          <div>
            ...
            <div className='w-full small-size-auth-input pb-5' >
                <MobileInput register={register("phone_number")} returnedCountry={(ele)=>{
                    setCountry(ele)
        ...
        <div className=' py-2'>
            <button type="submit" className='btn-main p-5 !w-full'>{t("submit")}</button>
        </div>
    </form> );
}
